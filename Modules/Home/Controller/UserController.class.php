...
class UserController extends CommonController {
    ...
    function information(){
    	if(IS_POST){
			...
			if (!empty($d['telephone'])) {
				if(!preg_match("/^1[3-5,8]{1}[0-9]{9}$/",$d['telephone'])){
					$json['error']['telephone'] = '请输入正确的手机号码！！';
				}				
			}		
		...
		$this->action=U('/information');
		$this->infor=M('Member')->where(array('member_id'=>session('user_auth.uid')))->find();
		$this->display();		
	}	
	...
	function order(){
		...
		$this->assign('xunlei',$data['xunlei']);

		$code = M('Code');
		$alreadygetstate = $code->field('get_state')->select();

		// array_merge_recursive($alreadygetstate);
		// array_unique($alreadygetstate);
		// $hashids = new \Lib\Hashids(C('PWD_KEY'), C('URL_ID'));
		// foreach ($alreadygetstate as $k => $v) {
		// 	$aloid = $hashids->encode($v['get_state']);
		// 	$alreadygetstate[$k]['lvoid'] = $aloid;
		// }
		
		$this->assign('alreadygetstate',$alreadygetstate);
    ...
    /*    $ordergoods = M('OrderGoods');
        ...
        $order_id = I('post.');
        $hashids = new \Lib\Hashids(C('PWD_KEY'), C('URL_ID'));
        $ids=$hashids->decode($order_id['order_id']);

       	$whereoid['order_id']= $ids[0];
        $showsnum = $ordergoods->where($whereoid)->getField('quantity');
        $coo = $code->field('code_id,code')->where('get_state = 0')->limit($showsnum)->select();*/

        /*foreach ($coo as $value) {
        	$vv=join(',',$vv);
			$temp[]=$vv;
        }
		dump($temp);*/
...
//        print_r($coo);exit;
      /*  $oid=$ids[0];
      	...
      	$wheregid['order_id'] = $oid;
        $ordergoods->where($wheregid)->setField('code_num',$string);
        foreach ($coo as $k => $v) {
	        $wherecoo['code'] = $v['code'];
	        $get_state = $code->where($wherecoo)->setField('get_state',$oid);
        }
    ...
    public function alreadyGo() {
        $code = M('code');
        $order_id = I('post.');
        $hashids = new \Lib\Hashids(C('PWD_KEY'), C('URL_ID'));
        $ids=$hashids->decode($order_id['order_id']);
       	$whereoid['get_state']= $ids[0];
        $alreadycoo = $code->field('code')->where($whereoid)->select();
       	echo json_encode($alreadycoo);
