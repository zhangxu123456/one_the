	...
	function show_order_page($member_id){
		...
		if(!empty($list)){   //订单是否为空
			...
			foreach ($list as $item => $v){
				...
				$order_dan=$order->where(array('order_id'=>$order_num,'order_status_id' => 1))->find();  //已经生成订单和付款成功

				$order_cun=$order->where(array('order_id'=>$order_num,'zhuang_state'=>1))->getField('xuliemas');   //查找这个订单号不为空和状态为1的,并把序列号取出

				if($order_cun){        //直接从订单表中取序列码
					$strture=strstr($order_cun,'|'); //判断序列码中有没有|
					if(!$strture){  //如果没有直接返回
						$xunlei=$order_cun;
					}else{			//如果有|需要分割
					...
					if(!empty($order_dan)){  //订单号不为空
						$quantity=$order_goods->where(array('order_goods_id'=>$order_dan['order_id']))->getField('quantity'); //商品表中取出购买商品的数量
