...
<html lang="en">
	...
	<a href="<?php echo U('Code/add');?>" class="btn btn-primary">新增</a>
			...
			<table class="table table-striped table-bordered table-hover">
				...
				<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
							...
							<?php if($data["state"] == 1): ?><td>已经使用</td>
							<?php else: ?>
								<td>有效</td><?php endif; ?>

							<td>
								<a  class="btn btn-xs btn-info" href='<?php echo U("Code/edit",array("id"=>$data["code_id"]));?>'>
									<i class="icon-edit bigger-120"></i>
								</a> 
								<a class="delete btn btn-xs btn-danger" href='<?php echo U("Code/del",array("id"=>$data["code_id"]));?>' >
