            ...
            <div class="ct  w100">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody class="text-center table-cell"  style="margin:50px 0;">
                            <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$data): $mod = ($k % 2 );++$k;?><tr>
                                    ...
                                    <td class="text-center">    
                                        <?php if($data['status'] == '已付款待发货'): ?><i class="special-num">
                                                  <p  data-toggle="modal" data-target="#myModal"  class="pop_org" date-title="领取激活码" data="" >领取激活码</p>
                                                    ...
                                                    <!--     <p id="toggle_two" data-toggle="modal" data-target="#myModal"  class="pop_org" date-title="领取激活码" onclick="changeAttr('<?php echo ($data["order_id"]); ?>','<?php echo U('/alreadyGo');?>')" >查看激活码</p> -->
                                            </i><?php endif; ?>
                                    </td>
...
<script type="text/javascript">
    ...
    /*function changeAttr(order_id,urls){
     var order_id=order_id,urls=urls;*/

    // if(urls =='/theguide/changeattr.html'){
    //     console.log($(this));
    //     $('this').css('display','none');
    //     $(this).parent().append("<p id='toggle_two' data-toggle='modal' data-target='#myModal'  class='pop_org' date-title='领取激活码' onclick='changeAttr('<?php echo ($data["order_id"]); ?>','<?php echo U('/alreadyGo');?>')' >查看激活码</p>")
    //     // $('#toggle_two').css('display','block');
    // }
    /*$.ajax({
     url:urls,
     type:'post',
     ...
     $('#modal-body').append('<div class="col_num_total">'+
     '<li class="col-sm-4">激活码</li>'+
     '<li class="col-sm-4">'+datas[i]['code']+'</li>'+
     '<li class="col-sm-4">复制</li>'+
     '</div>'
     )
     }
